...
export const Login = () => {
    ...
    const { 
        loginAttributes, email, password, setEmail, setPassword,
        errorMessage,
        errRef,
        setErrorMessage,
        setLoginAttributes, 
                    ...
                    <div className='w-1/2 h-full bg-sky-50 rounded-lg -px-4 sm:w-full md:w-full lg:w-3/4 xl:w-2/3 2xl:w-1/2'>   
                            <div className='flex flex-col items-center justify-center'>
                                ...
                                <div className='w-1/2 bg-sky-50 relative'>
                                    <p ref={errRef} className={errorMessage ? "text-red-700 font-Poppins flex justify-center items-center py-2" : "absolute -left-[9999px]"} aria-live="assertive">
                                        {errorMessage}
                                    </p>
                                    <form className='text-black' onSubmit={loginClicked}>
                                        <div className='py-9 flex flex-col space-y-9'> 
                                            ...
                                            <div className='relative flex-row items-center'>
                                                <FaEnvelope color='gray' className='absolute z-10 mx-3 top-4' />
                                                <label htmlFor='email' className='relative cursor-pointer'>
                                                    <input 
                                                        ...
                                                        onFocus={() => setLoginAttributes(prev => ({...prev, loginEmailFocus: true}))}
                                                        onBlur={() => setLoginAttributes(prev => ({...prev, loginEmailFocus: false}))}

                                                    />

                                                    <span id='email_indicator' 
                                                        className={`${loginAttributes.loginEmailFocus ? "font-Poppins text-black text-opacity-100 absolute left-5 -top-4 px-2 transition duration-200 bg-sky-50 input-text"
                                                        : "absolute -left-[9999px]"}`}>Email Address
                                                    </span>
                                                </label>
                                            </div>
                                            ...
                                            <div className='relative flex-row items-center'>
                                                <FaLock color='gray' className='absolute z-10 mx-3 top-4' />
                                                <label htmlFor='password' className='relative cursor-pointer'>
                                                    ...
                                                    />
                                                    <span className='font-Poppins text-white text-opacity-100 absolute left-5 top-12 px-2 transition duration-200 bg-sky-50 input-password'>Password</span>
                                                </label>
                                            </div>
                                    
                                            <div className='space-y-2'>
                                                <Link to={`/forgotpassword`}>
                                                    <p  className='font-Poppins my-0 text-sm text-blue-600 hover:underline'>Forgot your password ?</p>
                                                </Link>
                                                <p className='font-Poppins text-sm m-0'>
                                                    Don't have an account? <span className='text-sm text-blue-600 hover:underline'><Link to={`/signup`}>Register</Link></span> By Clicking here.
                                                </p>
                                            </div>

  ...
  const onUploadClicked = async (res) => {
    try {
      const response = await axios.post(
        `http://localhost:5000/questionsfromcsv`,
        res,
        {
      ...
      <div className={`w-full p-5`}>
        <div className={`flex flex-col justify-center items-center`}>
          ...
          <div className={`w-2/3 flex flex-col space-y-3  my-5 mx-auto`}>
            <label htmlFor="importCSV" className={`font-Poppins`}>
              Browse a file
            </label>
            <input
              type={`file`}
          ...
          <div
            className={`flex w-4/5 justify-end mx-auto py-5 rounded-none p-1`}
          >
            <button
              className={`btn-sm px-5 font-Poppins text-sm rounded-none flex space-x-1 justify-center items-center`}
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: "3rem" }}>
        {error
          ? error
          : data.map((col, idx) => <div key={idx}>{JSON.stringify(col)}</div>)}
      </div>
    </div>
  );
};
